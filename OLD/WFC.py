...
import colorama
#from Text2Matrix import convert_to_matrix
def convert_to_matrix(path):
    lines=[list(i) for i in open(path,'r').read().split('\n')]
    
    return lines
    
#from Matrix2Text import convert_to_file
def convert_to_file(matrix,path):
    lines=[''.join(i) for i in matrix]
    file='\n'.join(lines)
...
def WFC(dic,x=10,y=10,usedic=True):
    if usedic:
        dic=convert_to_matrix(dic)
    input_matrix=dic
    compatibilities, weights = parse_example_matrix(input_matrix)
    compatibility_oracle = CompatibilityOracle(compatibilities)
    model = Model((x, y), weights, compatibility_oracle)
    output = model.run()
    return output
#print(WFC(sample_matrix,x=7,y=7, usedic=False))
'''while True:
    
    sample_matrix=convert_to_matrix("Level0.txt")
    e=WFC(sample_matrix,x=30,y=30, usedic=False)
    print('GO')
    input()
    sample_matrix=convert_to_matrix("Level0.txt")
    convert_to_file(e,None)'''
