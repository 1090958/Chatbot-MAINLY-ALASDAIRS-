...
class dungeon:
    class room:
        def __init__(self, node:dungeongraphgen.dungeon.room, typ:roomtypes.roomtype.library|roomtypes.roomtype.regular):
            self.node=node
            self.description, self.enimies, self.items, self.rt=typ.describe(), typ.getenimies(), typ.getitems(), typ
    ...
    class newdungeon():
        def __init__(self, size:tuple[int,int], 
            ...
            self.graph=dungeongraphgen.dungeon.new_dungeon(size, dchance, spa)
            self.size=size
            self.spa=spa
            self.v=dchance
            
            self.rooms=[dungeon.room(i, [roomtypes.roomtype.library, roomtypes.roomtype.regular][random.randint(0,1)]) for i in self.graph.rooms]
